...
gsap.from(rectangles, {
 ...
 /* function initFeaturesMobileDropdown() {
  ...
  function initSolutions() {
    let content = document.querySelector('[data-solutions-hero-content]')
    let partnersContent = document.querySelector('[data-solutions-partners-content]')
    if(!content) {return;}
    let backgrounds = document.querySelectorAll('[data-solutions-background]')

    let tl = gsap.timeline()

    tl.from([content, partnersContent, backgrounds], {opacity: 0}, 0.5)
  }

  function scrollSolutionsCases() {
    let wrap = document.querySelector('[data-solutions-case-wrap]')
    let logos = document.querySelectorAll('[data-solution-case-logo]')
    if(logos.length === 0) {return;}

    let contents = document.querySelectorAll('[data-solution-case-content]')
    let scrollIndicator = document.querySelector('[data-solution-case-scroller]')
    ...
    gsap.set([logos[1], contents[1]], {autoAlpha: 0})

    tl.to(scrollIndicator, {y: "7rem", ease: "none", duration: 2})
    tl.to([logos[0], contents[0]], {autoAlpha: 0, ease: "none", duration: 0.5}, "<0.1")
    tl.to([logos[1], contents[1]], {autoAlpha: 1, ease: "none", duration: 0.5}, ">0.75")
