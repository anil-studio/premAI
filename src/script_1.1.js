...
function openNavMobile(){
  ...
  openBtn.addEventListener("click", () => {
    if (tl.progress() === 0) {
      ...
      // Ne pas modifier la position de la navbar lors de la fermeture
    }
  });
  ...
  window.addEventListener('scroll', () => {
    if (!mobileMenuOpened) {
      ...
      if (currentScrollPosition !== lastScrollPosition) {
        // L'utilisateur a fait défiler la page après la fermeture du menu
        lastScrollPosition = currentScrollPosition;
        // Réactiver le comportement normal de la navbar
